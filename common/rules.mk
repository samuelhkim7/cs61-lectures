...
GXX_ISCLANG := $(shell if g++ --version 2>/dev/null | grep -e 'LLVM\|clang' >/dev/null; then echo 1; else echo 0; fi)
 ifeq ($(GXX_ISCLANG),1)
  ifeq ($(shell if g++-14 --version 2>/dev/null | grep -e 'Free Software' >/dev/null; then echo 1; else echo 0; fi),1)
CXX_GCC = g++-14
  else ifeq ($(shell if g++-13 --version 2>/dev/null | grep -e 'Free Software' >/dev/null; then echo 1; else echo 0; fi),1)
CXX_GCC = g++-13
  else
CXX_GCC = false
  endif
